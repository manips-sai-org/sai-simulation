...
namespace SaiSimulation
{
	...
	public:
		...
		/**
		 * @brief      Gets the contact information, whenever a contact occurs at a given link
		 *
		 * @param[in]  link_name       The link name
		 * @return  a vector of ContactInfo structures, each containing:
		 * - model_name: the name of the robot or object in contact
		 * - link_name: the name of the link in contact
		 * - position: the position of the contact point in world coordinates
		 * - normal: the contact normal in world coordinates (pointing out of the
		 * object)
		 * - normal_force: the contact normal force vector in world coordinates
		 * - friction_force: the contact friction force vector in world coordinates
		 * - force_magnitude: the magnitude of the contact normal force
		 * - time: the time at which the contact was recorded
		 * NOTE: this function can be called at any time, but it will return
		 * contact information only if a contact occurred during the last
		 * integration step
		 */
		std::vector<Contact3d> getContactInfo(const std::string &robot_name,
